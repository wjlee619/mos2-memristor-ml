...
|------|---------|-----------|-------|------|
| `probe/` | 73개 | 2단자 ±10 V 서브-임계 스윕 | Phase 1 | ✅ |
...
## Phase 1 — 2단자 ±10 V 서브-임계 스윕 ✅

### 배경: 왜 이 데이터부터 시작했나

probe/ 폴더의 73개 파일은 MoS₂ 소자에 ±10 V를 인가한 **2단자 전압 스윕** 측정값이야. 4접촉 십자형 전극 구조에서 접촉 1번과 3번에 두 프로브를 연결해 측정했고, 게이트 단자는 없어.

**중요 — 이 스윕은 스위칭을 측정하지 않아:** ±10 V 범위는 SET 임계전압(12–19 V)보다 낮아서 소자가 스위칭되지 않아. 따라서 여기서 얻은 ON/OFF 값(~10⁰⁻¹)은 **서브-임계 전도 비율**이지, ±20 V memeffect 스윕에서 확인되는 ~10⁷ 수준의 멤리스터 스위칭 비율이 아니야.

**파일명 "vgs_sweep" 설명:** CSV 파일 이름에 "vgs"가 들어간 건 연구실 Keithley 스크립트 템플릿이 전계효과 트랜지스터(FET)용으로 만들어졌기 때문이야. 이 멤리스터 2단자 측정에도 같은 템플릿을 그대로 썼어. "vgs"는 측정 방식이 아니라 스크립트 이름의 흔적 — 이 소자에 게이트는 없어.

### 핵심 개념

**2단자 서브-임계 스윕**
- 두 단자 사이에 전압을 ±10 V로 스윕하면서 전류(Id)를 측정
- SET 임계전압 이하 → 소자 미스위칭 → 서브-임계 전도 특성 측정
- MoS₂ 레이어 수별 반복 측정 → 레이어 두께가 저전압 전도 특성에 미치는 영향 파악
- CSV 한 파일 = 실험 한 번
...
| Feature | 수식 | 의미 | 좋은 값 |
|---------|------|------|---------|
| `id_on_A` | max\|Id\| | 서브-임계 최대 전류 | 클수록 큰 서브-임계 전도 |
| `id_off_A` | min\|Id\| (0 제외) | 서브-임계 최소 전류 | 작을수록 낮은 누설 |
| `on_off_ratio` | id_on ÷ id_off | 서브-임계 전류 비율 | 참고값 (멤리스터 스위칭 비율 아님) |

**왜 이 세 가지냐:** 전류 비율로 레이어 수의 예측력을 평가하기 위한 지표. 단, 이 값들은 SET 임계전압 아래 서브-임계 전도 특성이지 멤리스터 스위칭 성능 지표가 아니야.
...
원인: Keithley ±10 V 고속 스윕 모드의 측정 분해능 한계 (fast-sweep noise floor)
참고: 느린 IV 스윕 모드에서는 ~pA 수준까지 측정 가능 — 모드별로 분해능이 다름
대응: ML feature로 id_off 쓸 때 이 모드 한계를 명시
```
...
Phase 1 스윕은 ±10 V 서브-임계 전도 특성을 봤어. 근데 이 소자의 핵심은 **멤리스터** — 메모리 특성이야. SET/RESET voltage를 추출해야 "어떤 조건에서 스위칭이 일어나나"를 ML로 분석할 수 있어.
